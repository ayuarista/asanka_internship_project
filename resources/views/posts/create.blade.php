<x-layout>
    ...
    <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" class="w-full p-2 border rounded focus:outline-indigo-500" value="{{ old('title') }}" required>
        </div>
        <div class="mb-4">
            <label class="block font-medium text-gray-900">Content</label>
            <textarea name="body" class="w-full p-2 border rounded focus:outline-indigo-500" required>{{ old('body') }}</textarea>
        </div>
        <div class="mb-4">
            <label class="block font-medium text-gray-900">Select Category</label>
            <select name="category_id" id="category" class="w-full border focus:outline-indigo-500 p-2 rounded-md">
        ...
        <div class="col-span-full">
            <div class="flex gap-2">
                ...
                <p class="text-red-600 text-sm">*max size 2MB</p>
            </div>
            <div class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
                <div class="mt-4 flex flex-col items-center text-sm text-gray-600">
                    <label for="imageInput"
                        class="relative cursor-pointer rounded-md bg-white border border-gray-300 px-4 py-2 text-indigo-600 font-semibold hover:bg-gray-100 hover:text-indigo-500">
                        Choose Image
                    </label>
                    ...
                    <p id="fileError" class="text-red-500 text-sm hidden mt-2">File terlalu besar! Maksimal 2MB.</p>
                </div>
            </div>
        </div>

        <button type="submit" class="bg-blue-500 hover:bg-blue-600 cursor-pointer mt-3 w-full text-white px-4 py-2 rounded">
            Create Post
        </button>
    </form>
